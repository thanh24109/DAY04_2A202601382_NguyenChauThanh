# Kế hoạch thực hiện Cá nhân (Solo) - Đề tài: Research Paper Scout 🔬
...
provider: none
requires_env: []
inputs: [title, authors, year, venue, url, arxiv_id]
outputs: [apa, bibtex, plain]
side_effect: false
---
# citation_generator

Generates academic citations in multiple formats (APA 7th edition, BibTeX, and plain text)
from paper metadata provided by the user or obtained from the `papers` tool.

## When to use

- User asks to "create a citation", "generate a reference", "cite this paper", or "get BibTeX"
- User provides paper title + authors, or an arXiv URL / ID
- After `papers` returns results and user wants to cite one of them

## When NOT to use

- User just wants to find papers (use `papers` instead)
- User wants to read full paper content (use `paper_text`)
- User wants a web search (use `lookup`)

## Argument conventions

- `title` (required): Full title of the paper
- `authors` (required): List of author names as a single string (e.g. "Vaswani, A., Shazeer, N.")
  or a list. If only last names known, use what's available.
- `year` (required): Publication year as integer or string (e.g. 2017 or "2017")
- `venue` (optional): Journal, conference, or "arXiv preprint" — default "arXiv preprint"
- `url` (optional): URL to the paper (abstract page)
- `arxiv_id` (optional): arXiv ID like "1706.03762" — used to build URL if not given

## Output fields

- `apa`: Citation in APA 7th edition format
- `bibtex`: BibTeX entry string
- `plain`: Short plain-text reference (author, year, title)
